...
describe('Libs: JSONAPI', function () {
  ...
  var result_from_api, result_from_api_one_recourse
  ...
  beforeEach(function() {
    ...
    result_from_api_one_recourse = clone(result_from_api)

    result_from_api_one_recourse.data = result_from_api.data[0]
  ...
  it('if the api respond a resource with a meta move meta from outside the data to inside the data', function() {
    var result = clone(Object.assign(
      {},
      result_from_api_one_recourse,
      {
        meta: {
    ...
    expect(parsed_result.data.meta).toEqual(result.meta)
  })
